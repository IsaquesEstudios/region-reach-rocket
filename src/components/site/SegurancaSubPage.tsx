...
import { type SegurancaSubservice, segurancaSubservices, whatsappLink } from "@/lib/site";
import { QuoteForm } from "./QuoteForm";

interface Props {
  data: SegurancaSubservice;
  image: string;
}

export function SegurancaSubPage({ data, image }: Props) {
  const related = segurancaSubservices.filter((s) => data.related.includes(s.slug));
    ...
    <>
      <nav aria-label="Breadcrumb" className="bg-surface border-b border-border">
        <ol className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-wrap items-center gap-2 text-xs font-medium text-muted-foreground">
          ...
          <li aria-hidden>/</li>
          <li><Link to="/servicos/seguranca" className="hover:text-primary">Segurança</Link></li>
          <li aria-hidden>/</li>
      ...
      <section className="pt-12 sm:pt-16 pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-12 gap-10 items-end">
          <div className="lg:col-span-7">
            <span className="inline-block rounded-full bg-primary/10 text-primary px-3 py-1 text-[11px] font-semibold uppercase tracking-widest">{data.code} · Segurança · {data.shortTitle}</span>
          ...
          <div className="lg:col-span-5">
            <img
              src={image}
              alt={`${data.h1} — Chico Resolve, empresa especializada em segurança eletrônica em Fortaleza`}
              width={1280}
              height={960}
              loading="eager"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2">
            ...
            <div className="mt-16">
              ...
              <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight mb-6">
                Normas e padrões que aplicamos
              </h2>
          ...
          <aside className="lg:sticky lg:top-28 h-fit">
            <div className="rounded-2xl bg-card p-7 border border-border">
              <h2 className="text-xl font-extrabold mb-2">Orçamento de segurança</h2>
      ...
      <section className="py-16 border-t border-border" aria-label="Outras especialidades em segurança eletrônica">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <p className="text-xs font-semibold text-primary uppercase tracking-widest mb-3">Veja também</p>
          <h2 className="text-3xl font-extrabold tracking-tight mb-10">Outras especialidades em segurança</h2>
          ...
          <div className="mt-10">
            <Link to="/servicos/seguranca" className="inline-block rounded-full bg-primary/10 text-primary px-3 py-1 text-[11px] font-semibold uppercase tracking-widest hover:underline">
              ← Voltar ao hub de segurança
            </Link>
          </div>
        </div>
      </section>

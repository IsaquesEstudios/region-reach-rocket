...
import { type ReformaSubservice, reformaSubservices, whatsappLink } from "@/lib/site";
import { QuoteForm } from "./QuoteForm";

interface Props {
  data: ReformaSubservice;
  image: string;
}

export function ReformasSubPage({ data, image }: Props) {
  const related = reformaSubservices.filter((s) => data.related.includes(s.slug));
    ...
    <>
      <nav aria-label="Breadcrumb" className="bg-surface border-b border-border">
        <ol className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-wrap items-center gap-2 text-xs font-medium text-muted-foreground">
          ...
          <li aria-hidden>/</li>
          <li><Link to="/servicos/reformas" className="hover:text-primary">Reformas</Link></li>
          <li aria-hidden>/</li>
      ...
      <section className="pt-12 sm:pt-16 pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 grid lg:grid-cols-12 gap-10 items-end">
          <div className="lg:col-span-7">
            <span className="inline-block rounded-full bg-primary/10 text-primary px-3 py-1 text-[11px] font-semibold uppercase tracking-widest">{data.code} · Reformas · {data.shortTitle}</span>
            <h1 className="mt-4 text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-[1.05] text-balance">
              {data.h1.replace(" em Teresina", "").replace(" em Teresina/PI", "")} em <span className="text-primary">Teresina</span>
            </h1>
          ...
          <div className="lg:col-span-5">
            <img
              src={image}
              alt={`${data.h1} — Chico Resolve, empresa de reformas em Teresina`}
              width={1280}
              height={960}
              loading="eager"
      ...
      <section className="py-16 border-t border-border" aria-label="Outras especialidades em reformas">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <p className="text-xs font-semibold text-primary uppercase tracking-widest mb-3">Veja também</p>
          <h2 className="text-3xl font-extrabold tracking-tight mb-10">Outras especialidades em reformas</h2>
          ...
          <div className="mt-10">
            <Link to="/servicos/reformas" className="inline-block rounded-full bg-primary/10 text-primary px-3 py-1 text-[11px] font-semibold uppercase tracking-widest hover:underline">
              ← Voltar ao hub de reformas
            </Link>
          </div>
        </div>
      </section>
